...
class MoEAdapter(nn.Module):
    def __init__(self, num_experts, top_k, config, d_model=None, bottleneck=None, dropout=0.1):
        super().__init__()
...
class VisionTransformer(nn.Module):
    ...
    def init_adapters(self):
        ...
        for _ in range(len(self.blocks)):
            moe_adapter = MoEAdapter(num_experts=self.config.num_experts, top_k=self.config.top_k,
                                     config=self.config, dropout=0.1, bottleneck=self.config.ffn_num
                                     ).to(self._device)
        ...
        for layer_idx in range(len(self.blocks)):
            for expert_idx in range(self.config.num_experts):
                ...
                with torch.no_grad():
                    ...
                    # Get current expert's parameters
                    for name, param in current_expert.named_parameters():
                        if param.requires_grad:  # Only merge trainable parameters
                            past_params = []    # Collect corresponding parameters from past experts
                            for past_param_dict in past_param_dicts:
                                if name in past_param_dict:
                                    past_params.append(past_param_dict[name].data)

                            if past_params:  # Only merge if we found matching parameters
                                avg_param = torch.stack(past_params, dim=0).mean(dim=0)
...
def vit_base_patch16_224_in21k_mos_moe(pretrained=False, **kwargs):
    model = VisionTransformer(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
